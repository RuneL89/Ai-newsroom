...
export function parseFullScriptEditorOutput(raw: string): AuditResult {
  ...
  try {
    ...
    if (objectMatch) {
      try {
        ...
        throw new Error('Failed to parse Full Script Editor audit JSON. Raw output:\n' + raw.slice(0, 500));
      }
    } else {
      throw new Error('Failed to parse Full Script Editor audit JSON. Raw output:\n' + raw.slice(0, 500));
    }
  ...
  if (!parsed || typeof parsed !== 'object') {
    throw new Error('Full Script Editor output is not a JSON object');
  }
  ...
  // Validate stories array if present (backward compatibility)
  let stories: AuditResult['stories'] = [];
  if (Array.isArray(obj.stories)) {
    stories = obj.stories.map((story: unknown, idx: number) => {
